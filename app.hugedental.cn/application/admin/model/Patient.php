...
class Patient extends Model
{
	...
	public function get_last_patient($user_id, $cloumn)
	{
  	...
  	//查询患者
  	public function get_patient_list($param=array())
	{
		//$obj_data = Db::name('hg_patient')->alias('a')->join('hg_user b', 'a.operate_user_id = b.user_id');
		
		if (!empty($param['keyword']))
		{
          	$sql = "SELECT * FROM hg_patient a LEFT JOIN hg_user b ON a.operate_user_id = b.user_id WHERE a.name LIKE '%".$param['keyword']."%' OR a.hospital LIKE '%".$param['keyword']."%' OR a.production_unit LIKE '%".$param['keyword']."%' OR b.username LIKE '%".$param['keyword']."%' OR b.user_id LIKE '%".$param['keyword']."%' OR a.mobile LIKE '%".$param['keyword']."%'";
			//$obj_data = $obj_data->where('a.name', 'like', '%'.$param['keyword'])->whereOr('a.hospital', 'like', '%'.$param['keyword'])->whereOr('a.production_unit', 'like', '%'.$param['keyword'])->whereOr('username', 'like', '%'.$param['keyword'])->whereOr('user_id', 'like', '%'.$param['keyword'])->whereOr('a.mobile', 'like', '%'.$param['keyword']);
          	$res = Db::query($sql);
          	return $res;
		}
      	else
