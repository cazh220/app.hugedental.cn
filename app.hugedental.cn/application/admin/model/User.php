...
class User extends Model
{
	public function insert_user($data=array())
	{
		...
		if (!empty($data))
		{
			...
			//$sql = rtrim($sql, ',');//echo $sql;die;
          	$sql .= " create_time = NOW()";//echo $sql;die;
			$res = Db::execute($sql);
          
          	$userId = Db::name('hg_user')->getLastInsID();
          	
          	$y = substr(date("Y", time()), -1 ,1);
          	$len = strlen(floor($user_id));
          	$m = '';
          	for($i=0; $i<5-$len; $i++)
            {
            	$m .= '0';
          	...
          	$m = 'F'.$y.$m.$userId;
          	$sql = "UPDATE hg_user SET member_id = '".$m."' WHERE user_id = ".$userId;//echo $sql;die;
          	$res = Db::execute($sql);
...
//		}
  	...
  	public function user_search($search='')
    {
    	...
    	if($search)
        {
        	$sql = "SELECT * FROM hg_user WHERE user_type = 1 AND status = 1 AND (member_id LIKE '%".$search."%' OR company_name LIKE '%".$search."%' OR realname LIKE '%".$search."%' OR mobile LIKE '%".$search."%')";
	...
	public function getHistory($user_id = 0)
	{
  	...
  	//获取技工录入的数量
  	public function get_tooth_num($user_id)
    {
      	$res = array();
    	if($user_id)
        {
        	$sql = "SELECT SUM(tooth_num) as 'total' FROM hg_patient WHERE operate_user_id = ".$user_id;
          	$res = Db::query($sql);
      	...
      	return !empty($res[0]['total']) ? $res[0]['total'] : 0; 
    }
