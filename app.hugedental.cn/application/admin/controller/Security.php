...
class Security 
{
    ...
    {	
		...
		if($list['data'])
		{
			...
			{
              	$list['data'][$key]['qrcode'] = urlencode("http://fangwei.hugedental.cn/index.php?do=share&security_code=".$val['security_code']);
				//$list['data'][$key]['qrcode'] = urlencode("http://fangwei.hugedental.cn/index.php?do=middlepage&security_code=".$val['security_code']);
			}
		}
    ...
    public function export_security_code()
    {
        ...
        {
            foreach($list as $k => $v){
              	$url_encode = urlencode("http://fangwei.hugedental.cn/index.php?do=share&security_code=".$v["security_code"]);
            	//$url_encode = urlencode("http://fangwei.hugedental.cn/index.php?do=middlepage&security_code=".$v["security_code"]);
            	//$url = "http://qr.liantu.com/api.php?text=".$url_encode;
            	//$url = "http://fangwei.hugedental.cn/index.php?do=middlepage&security_code=".$v["security_code"];
              	$url = "http://fangwei.hugedental.cn/index.php?do=share&security_code=".$v["security_code"];
                $num=$k+2;
              	$code_id = $this->clean_param($v['code_id']);
              	$security_code = $this->clean_param($v['security_code']);
              	$security_code = $this->clean_param($v['security_code']);
              	$url = $this->clean_param($url);
              	
                $objPHPExcel->setActiveSheetIndex(0)//Excel的第A列，uid是你查出数组的键值，下面以此类推
                ->setCellValue('A'.$num, $code_id)
                ->setCellValue('B'.$num, $security_code.'')
                ->setCellValue('C'.$num, $security_code.'')
	...
	{
		$data = $data ? trim($data) : '';
		$qrdata = urlencode("http://fangwei.hugedental.cn/index.php?do=share&security_code=".$data);
...
//		}
	...
	public function export_qrcode($list)
	{
		...
		foreach($list['data'] as $key => $val)
		{
			if($key < 50)
			{
				$url_encode = urlencode("http://fangwei.hugedental.cn/index.php?do=share&security_code=".$val["security_code"]);
    ...
    public function import_act()
    {
	    ...
	    if($file){
	        ...
	        if($info){
	            ...
	            {
	            	//txt
	            	$res = $this->import_txt($file_save, $them, $production_title, $prefix);
	            }
	            else
	            {
	            	//excel
	            ...
	            if($res)
	            {
	            	echo "<script>alert('导入成功');window.location.href='./create_list';</script>";
	            }
	            else
	            {
    ...
    //txt导入
    public function import_txt($file, $them, $production_title, $prefix)
    {
