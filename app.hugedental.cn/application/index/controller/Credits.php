	...
	public function do_edit_member()
	{
		...
		else
		{
          	...
          	if($res)
            {
              	...
              	$fix_type = $Security->get_fix_type($_POST['security_code']);
              	
              	$Credit = model('Credits');
              	if($fix_type==1)
                {
                  	$score = $Credit->get_credit(8);
                }
              	elseif($fix_type==4)
                {
                  	$score = $Credit->get_credit(9);
                }
              	//获取防伪码的4分
            	$credit_flow = array(
                	'type'	=> 1,
              	...
              	$Security->insert_credit_flow($credit_flow);
		...
		{
			echo "<script>alert('录入成功');window.location.href='http://app.hugedental.cn/public/index.php/index/credits/write_security_code';</script>";
