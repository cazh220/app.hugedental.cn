...
class Index
{
	...
	public function do_register()
	{
		...
		else
		{
			$param['head_img'] = $upload['image'];
          	$param['upload_head_img'] = $upload['image'];
		}
		...
		if(empty($res))
		{
      	...
      	$content = "祝贺您成为沪鸽防伪认证中心会员！如果您在使用本系统的过程中遇到问题，请查看个人中心备查资料的常见问题解答；如果需要人工客服，请您在沪鸽微信公众平台沟通！";
		$User->insert_message($mobileArr, $content);
	...
	public function rule()
	{
		//获取协议
      	$Member = Model("Member");
      	$info = $Member->get_xieyi();
      	
		$view = new View();
      	$view->assign('xieyi', !empty($info[0]['content']) ? $info[0]['content'] : '');
		$view->assign('user', Session::get('user.mobile'));
