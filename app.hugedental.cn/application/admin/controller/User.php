...
class User
{
		...
		if($data['data'])
		{
			...
			foreach($data['data'] as $key => $val)
			{
				...
				$data['data'][$key]['stock_count'] = $Stock->stock_out_num($val['mobile']);
              	...
              	if($val['user_type'] == 1)
                {
                	$num = $User->get_tooth_num($val['user_id']);
                  	$data['data'][$key]['tooth_count'] = $num;
                }
              	else
                {
                	$data['data'][$key]['tooth_count'] = 0;
                }
			}
		}
  	...
  	public function search()
    {
    	$search = $_GET['search'];
      	...
      	$user_detail = $User->user_search($search);
      
      	exit(json_encode(array('list'=>$user_detail)));
